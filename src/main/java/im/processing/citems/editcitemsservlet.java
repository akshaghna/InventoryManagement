...
public class editcitemsservlet extends HttpServlet {

	
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// TODO Auto-generated method stub
		System.out.println("editing catalogue item");
		...
		if(cataloguename==null || itemname==null || pcost <=0 || scost<=0)
		{
			...
			request.getSession().setAttribute("citemAction","FAIL");
			RequestDispatcher req = request.getRequestDispatcher("/jsp/catalogueitems/editcitem.jsp?itemname="+ itemname );
			req.forward(request, response);
		}
		else
		{
			boolean result = DBUtils.editcitem(username, cataloguename,itemname,
			...
			if(result) {
				request.getSession().setAttribute("citemAction","SUCCESS");
				RequestDispatcher req = request.getRequestDispatcher("/jsp/catalogueitems/editcitem.jsp?itemname="+ itemname);	
				req.forward(request, response);
			}
			else {
